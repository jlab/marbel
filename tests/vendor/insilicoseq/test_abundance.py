...
from marbel.vendor.insilicoseq import abundance, util


@pytest.fixture
def setup_and_teardown():
    yield
    util.cleanup(["tests/vendor/insilicoseq/data/test_abundance.txt"])


def test_parsing():
    abundance_dic = abundance.parse_abundance_file("tests/vendor/insilicoseq/data/abundance.txt")
    assert abundance_dic == {"genome_ATCG": 0.1, "genome_TA": 0.1, "genome_A": 0.2, "genome_GC": 0.4, "genome_T": 0.2}


def test_parsing_readcounts():
    readcount_dic = abundance.parse_readcount_file("tests/vendor/insilicoseq/data/readcounts.txt")
    assert readcount_dic == {"amplicon_ATCG": 1, "amplicon_TA": 1, "amplicon_A": 2, "amplicon_GC": 4, "amplicon_T": 2}


def test_parsing_empty():
    with pytest.raises(SystemExit):
        abundance.parse_abundance_file("tests/vendor/insilicoseq/data/empty_file")


def test_parsing_no_exists():
    with pytest.raises(SystemExit):
        abundance.parse_abundance_file("tests/vendor/insilicoseq/data/does_not_exist")


def test_parsing_bad_abundance():
    with pytest.raises(SystemExit):
        abundance.parse_abundance_file("tests/vendor/insilicoseq/data/bad_abundance.txt")
...
def test_distributions():
    np.random.seed(42)
    f = open("tests/vendor/insilicoseq/data/genomes.fasta", "r")
...
def test_abunance_draft(setup_and_teardown):
    ...
    draft_genomes = ["tests/vendor/insilicoseq/data/draft.fasta"]
    ab = abundance.draft(complete_genomes, draft_genomes, abundance.lognormal, "tests/vendor/insilicoseq/data/test")
...
def test_coverage_scaling():
    abundance_dic = abundance.parse_abundance_file("tests/vendor/insilicoseq/data/abundance.txt")
    coverage_dic = abundance.coverage_scaling(10000, abundance_dic, "tests/vendor/insilicoseq/data/genomes.fasta", 25)
    assert coverage_dic == {
