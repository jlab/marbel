...
def setup_and_teardown():
    yield
    util.cleanup(["tests/vendor/insilicoseq/data/test_concat.iss.tmp.genomes.fasta"])


@pytest.fixture
def setup_and_teardown_compress():
    yield
    util.cleanup(["tests/vendor/insilicoseq/data/ecoli.fasta.gz"])
...
def test_count_records_empty():
    with pytest.raises(SystemExit):
        util.count_records("tests/vendor/insilicoseq/data/empty_file")
...
def test_genome_file_exists():
    with pytest.raises(SystemExit):
        my_important_file = "tests/vendor/insilicoseq/data/ecoli.fasta"
        util.genome_file_exists(my_important_file)


def test_reservoir():
    samples = []
    genome_file = "tests/vendor/insilicoseq/data/genomes.fasta"
...
def test_reservoir_invalid_input():
    with pytest.raises(SystemExit):
        genome_file = "tests/vendor/insilicoseq/data/ecoli.fasta"
...
def test_concatenate(setup_and_teardown):
    genome_files = ["tests/vendor/insilicoseq/data/ecoli.fasta"] * 2
    util.concatenate(genome_files, "tests/vendor/insilicoseq/data/test_concat.iss.tmp.genomes.fasta")
    with open("tests/vendor/insilicoseq/data/test_concat.iss.tmp.genomes.fasta", "rb") as f:
