...
def generate_dataset(n_species, n_orthogroups, n_samples, outdir, max_phylo_distance, min_identity, dge_ratio, seed,
    ...
    if group_orthology_level == OrthologyLevel.normal:
        species = dg.draw_random_species(number_of_species, rng)
    elif group_orthology_level == OrthologyLevel.very_low or group_orthology_level == OrthologyLevel.low:
        species = select_species_with_criterion(number_of_species, threads, seed, SelectionCriterion.minimize)
    else:
        species = select_species_with_criterion(number_of_species, threads, seed, SelectionCriterion.maximize)
    ...
    if group_orthology_level == OrthologyLevel.very_low:
        ...
        selected_ortho_groups = dg.draw_orthogroups(filtered_orthog_groups, number_of_orthogroups, species, rng, force=force_creation)
    else:
        selected_ortho_groups = dg.draw_orthogroups_by_rate(filtered_orthog_groups, ortho_group_rates, species, rng)
    ...
    if min_sparsity > 0:
        gene_summary_df = add_extra_sparsity(gene_summary_df, min_sparsity, rng)
    ...
    add_counts_to_large_orthogroups(gene_summary_df, number_of_species, rng)
